...
clim.vars <- c("rainfall_spr", "sun_spr", "tasmax_spr","tasmin_win")
...
writeRaster(p, file=paste0(wd.out,"/final_preds_t_sampled_nddlrd.tiff"))
...
writeRaster(test, file=paste0(wd.out,"/final_preds_t_sampled_nddlrd_se.tiff"))
...
r # 0.5799 ## why is this so much lower than the values for the validation data, dat.v? Presumably because dat.v contains a subset of the data. 

df <- data.frame(spec=r$specificities, sens=r$sensitivities, thresh=r$thresholds)

df[which.min(abs(0.95-df$sens)), ] ## The threshold that yields 95% sensitivity: 0.354047
df[which.min(abs(df$spec-df$sens)), ] ## The threshold that minimises the difference between sensitivity and specificity: 0.7455905
df[which.max(df$spec+df$sens), ]## minimises difference between sensitivity and specificity 0.7617743

### Reclassify prediction raster according to two thresholds (from pa_model3_eval_ply)
## 95% sensitivity threshold
rcl <- matrix(c(0,0.354047,0,   0.354047,1,1), nrow=2, byrow=T) 
p95 <- reclassify(p, rcl)
writeRaster(p95, file=paste0(wd.out,"/final_pred95_t_sampled_nddlrd.tiff"))

## Threshold that minimises difference between sensitivity and specificity
rcl <- matrix(c(0,0.7617743,0,   0.7617743,1,1), nrow=2, byrow=T) 
pss <- reclassify(p, rcl)
writeRaster(pss, file=paste0(wd.out,"/final_predss_t_sampled_nddlrd.tiff"))
...
load(paste0(wd.out,"/linear_quadratic_interactions_all_t_sampled12_nddlrd_AIC2")) ## All the best models were in the second subset, and as the files were so big I saved just the best model subset on this computer. All the other files are on teh cluster
...
rsq2 <- unlist(lapply(mlqid.comb, rsq.fn))
