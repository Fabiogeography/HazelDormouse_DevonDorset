...
load(file=paste0(wd.out,"/mods_final_surv-abs")) ## This model averages all constituent models. Probably not meaningful for the interaction variables selected only a few times. 
...
roc_obj <- roc(dat.v$occ, p.valid, plot=T)
auc(roc_obj) ## 0.8096
...
p <- predict(env.r, final, re.form=NA, type="response", full=T, se.fit=F)
writeRaster(p, file=paste0(wd.out,"/final_preds_t_sampled.tiff"))
...
writeRaster(test, file=paste0(wd.out,"/final_preds_t_sampled_se.tiff"))
...
r #  0.6661 ## why is this so much lower than the values for the validation data, dat.v? Presumably because dat.v contains a subset of the data. 

df <- data.frame(spec=r$specificities, sens=r$sensitivities, thresh=r$thresholds)

df[which.min(abs(0.95-df$sens)), ] ## The threshold that yields 95% sensitivity: 0.3720899
df[which.min(abs(df$spec-df$sens)), ] ## The threshold that minimises the difference between sensitivity and specificity: 0.8071694
df[which.max(df$spec+df$sens), ]## minimises difference between sensitivity and specificity 0.8462111

### Reclassify prediction raster according to two thresholds (from pa_model3_eval_ply)
## 95% sensitivity threshold
rcl <- matrix(c(0,0.3720899,0,   0.3720899,1,1), nrow=2, byrow=T) 
p95 <- reclassify(p, rcl)
writeRaster(p95, file=paste0(wd.out,"/final_pred95_t_sampled.tiff"))

## Threshold that minimises difference between sensitivity and specificity
rcl <- matrix(c(0,0.8462111,0,   0.8462111,1,1), nrow=2, byrow=T) 
pss <- reclassify(p, rcl)
writeRaster(pss, file=paste0(wd.out,"/final_predss_t_sampled.tiff"))
...
# load(paste0(wd.out,"/linear_quadratic_interactions_all_t_sampled1"))
load(paste0(wd.out,"/linear_quadratic_interactions_all_t_sampled123")) ## All the best models were in the second subset, and as the files were so big I saved just the best model subset on this computer. All the other files are on teh cluster
# load(paste0(wd.out,"/linear_quadratic_interactions_all_t_sampled3"))

# mlqid.1 <- get.models(mlqid.1, subset=delta<2) ## mlqid.1 is an object containing the best models. 
mlqid.2 <- get.models(test, subset=delta<2) ## mlqid.2 is an object containing the best models. 
...
mean(rsq2) #c(rsq1, rsq2, rsq3)) ## 0.2083371
sd(rsq2) #c(rsq1, rsq2, rsq3)) ## 0.009964297
