...
# write.csv(dat.m, paste0(wd.out, "/dat_std_22Feb2022_t_m_sampled_nddlrd_pabs.csv"), row.names=F)
dat.m <- read.csv(paste0(wd.out, "/dat_std_22Feb2022_t_m_sampled_nddlrd_pabs.csv"))

### Calibration and validation data to make the model
# dat.c <- sample_frac(cbind(dat.m), size=0.7)
# write.csv(dat.c, paste0(wd.out, "/dat_std_22Feb2022_t_c_sampled_nddlrd_pabs.csv"), row.names=F)
dat.c <- read.csv(paste0(wd.out, "/dat_std_22Feb2022_t_c_sampled_nddlrd_pabs.csv"))

# dat.v <- dat.m[!(dat.m$id %in% dat.c$id),]
# write.csv(dat.v, paste0(wd.out, "/dat_std_22Feb2022_t_v_sampled_nddlrd_pabs.csv"), row.names=F)
dat.v <- read.csv(paste0(wd.out, "/dat_std_22Feb2022_t_v_sampled_nddlrd_pabs.csv"))
...
Weights(mlqid.comb[mlqid.comb$delta<=2,])

#################################################
###### Produce & save final model ###############
#################################################
load(file=paste0(wd.out, "/linear_quadratic_interactions_all_t_sampled_nddlrd_pabs_best"))

mlqid.comb.mods <- get.models(mlqid.comb, subset=delta<2)
...
vars <- names(final.coefs) ## 26 variables, even though some interactions appear in a very small number of models.
coefs <- as.vector(final.coefs)
final.coefs <- data.frame(Variables=vars, MeanCoefficients=coefs)
rownames(final.coefs) <- vars

## Write outputs
save(final, file=paste0(wd.out,"/mods_final_t_sampled_nddlrd_pabs"))
write.csv(final.coefs, file=paste0(wd.out,"/mods_final_t_sampled_nddlrd_pabs.csv"), row.names=F)
