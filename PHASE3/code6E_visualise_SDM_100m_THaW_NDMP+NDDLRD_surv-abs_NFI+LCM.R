##############################################################
##### EVALUATE MODELS ########################
##### Best model made with NDMP & NDD+LRD presences and survey absences, but where treehedge (and frag) were not selected and NFI_LCM broadleaf forest was #####
...
##############################################################
final.coefs <- read.csv(file=paste0(wd.out,"/mods_final_surv-abs_NFI+LCM.csv"))
...
vars.names <- c("Ancient woodland", "NFI + LCM broadleaf coverage", "Spring rainfall",
                "Scrub", "Spring sun", "Spring max temp", 
                "Winter min temp", "Winter temp range")
...
png(paste0(wd.out, "/final_response_curves_avg_t_NFI+LCM.png"), width=700, height=500)
par(mfrow=c(3,3))
...
jpeg(paste0(wd.out,"/int_t_NFILCM_scrub_tasmax.jpg"), width=600, height=400)
interact_plot(dummy, pred=scrub, modx=tasmax_spr_5yrmn, interval=T, rug=T)
dev.off()

jpeg(paste0(wd.out,"/int_t_NFILCM_scrub_tasmin.jpg"), width=600, height=400)
