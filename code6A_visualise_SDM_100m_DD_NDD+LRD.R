...
##### Modified on: 10th November 2021  #########################
##############################################################
...
##############################################################
final.coefs <- read.csv(file=paste0(wd.out,"/mods_final_nddlrd.csv"))
...
vars.names <- c("NFI+LCM broadl coverage", "NFI+LCM conif change",
                "Slope %", "Min winter temperature", "Aspect NS", 
                "NFI+LCM broadl change", "Aspect WE", "NFI+LCM conif coverage")
...
png(paste0(wd.out, "/final_response_curves_avg_nddlrd.png"), width=700, height=500)
par(mfrow=c(2,4))
...
jpeg(paste0(wd.out,"/int_broadl_m_conif_C.jpg"), width=600, height=400)
interact_plot(dummy, pred=combo_broadl_m, modx=combo_conif_c, interval=T, rug=T)
dev.off()

jpeg(paste0(wd.out,"/int_broadl_m_slope.jpg"), width=600, height=400)
interact_plot(dummy, pred=combo_broadl_m, modx=OS_Terrain_100_slope_pct, interval=T, rug=T)
dev.off()

jpeg(paste0(wd.out,"/int_broadl_m_tasmin.jpg"), width=600, height=400)
interact_plot(dummy, pred=combo_broadl_m, modx=tasmin_win_5yrmn, interval=T, rug=T)
dev.off()

jpeg(paste0(wd.out,"/int_broadl_m_NSaspect.jpg"), width=600, height=400)
interact_plot(dummy, pred=combo_broadl_m, modx=OS_Terrain_100_NS, interval=T, rug=T)
dev.off() ## miniscule
