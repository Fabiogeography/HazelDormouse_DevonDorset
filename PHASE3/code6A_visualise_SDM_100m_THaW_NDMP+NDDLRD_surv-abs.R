##############################################################
##### EVALUATE MODELS ########################
##### Models made with NDMP & NDD+LRD presences and survey absences #####
...
##############################################################
final.coefs <- read.csv(file=paste0(wd.out,"/mods_final_surv-abs.csv"))
...
vars.names <- c("Ancient woodland", "Spring rainfall",
                "Scrub", "Spring sun", "Spring max temp", 
                "Winter min temp", "Winter temp range", "THaW trees+hedge",
                "Aspect WE", "Aspect NS", "Slope %")
...
png(paste0(wd.out, "/final_response_curves_avg_t.png"), width=700, height=500)
par(mfrow=c(3,4))
